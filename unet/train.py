...
def train_model(
    ...
    # (Initialize logging)
    experiment = wandb.init(project='U-Net', resume='allow', anonymous='must', entity="pauldd")
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                assert integral_images.shape[1] == model.in_channels, \
                    f'Network has been defined with {model.in_channels} input channels, ' \
                    f'but loaded images have {integral_images.shape[1]} channels. Please check that ' \
...
if __name__ == '__main__':
    ...
    model = UNet(in_channels=args.in_channels, out_channels=1, bilinear=args.bilinear)
