...
def main():
    ...
    enc = Dinov2(dinov2_size="small")  # here is the place to init the model(s)
    dec = DPT(embed_dims=384, channels=48, post_process_channels=[48, 96, 192, 384])
    pl_module = RestorationLitModule(optimizer=AdamW, scheduler=ReduceLROnPlateau, compile=False, encoder=enc, decoder=dec)
