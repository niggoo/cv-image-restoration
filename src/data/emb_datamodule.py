...
class DinoEmbeddingDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        embeddings = []
        for embedding in sample["embeddings"]:
            with safe_open(embedding, framework="pt", device="cpu") as f:
                tensors = {}
                for key in f.keys():
        ...
        embeddings_stacked = torch.stack([e['last_hidden_states'][:, 1:, :] for e in embeddings], dim=0).squeeze().float()

        return embeddings_stacked, gt / 255.0  # "normalize" to [0, 1]

class DinoEmbeddingDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        self.data_train: Optional[Dataset] = DinoEmbeddingDataSet(data_paths[:train_size])
        self.data_val: Optional[Dataset] = DinoEmbeddingDataSet(data_paths[train_size:train_size + val_size])
        self.data_test: Optional[Dataset] = DinoEmbeddingDataSet(data_paths[train_size + val_size:])
