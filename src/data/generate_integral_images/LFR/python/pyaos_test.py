...
class TestAOSRenderTwice(unittest.TestCase):
    ...
    def setUp(self):
        self._window = pyaos.PyGlfwWindow(
            512, 512, "AOS"
        )  # make sure there is an OpenGL context
    ...
    def render_single_image(self, _aos):
        ...
        self.assertTrue(
            np.allclose(rimg2, np.ones(shape=(512, 512), dtype=np.float32) * 1.5)
        )
        self.assertTrue(
            np.allclose(img[:, :, 0], rimg[:, :, 0])
        )  # check that rimg has not changed!
        ...
        self.assertTrue(
            np.allclose(rimg, np.ones(shape=(512, 512), dtype=np.float32) * 2.0)
        )
        ...
        self.assertTrue(
            np.allclose(rimg, np.ones(shape=(512, 512), dtype=np.float32) * 3.0)
        )
...
if __name__ == "__main__":
    ...
    os.chdir(
        parent
    )  # change to AOS working dir for startup (this is required so that the program finds dlls and the shader)
