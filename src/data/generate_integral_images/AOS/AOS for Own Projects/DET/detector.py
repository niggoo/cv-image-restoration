...
try:
    ...
    """
    print(
        'ERROR: OpenVino is not initialized. Make sure it is installed and the environment variables are set \n \
...
class Detector:
    ...
    def init(self, model, device="CPU"):
        ...
        assert (
            len(self.net.input_info) == 1
        ), "Sample supports only YOLO based single input topologies"
        ...
        if self.net.input_info[self.input_blob].input_data.shape[1] == 3:
            self.input_height, self.input_width = self.net.input_info[
                self.input_blob
            ].input_data.shape[2:]
    ...
    def detect(self, image, prob_threshold=0.01, iou_threshold=0.25):
        ...
        infer_request_handle = self.exec_net.start_async(
        ...
        for obj in objects:
            ...
            cv2.rectangle(
                image, (obj["xmin"], obj["ymin"]), (obj["xmax"], obj["ymax"]), color, 2
            )
            cv2.putText(
                image,
...
if __name__ == "__main__":
    ...
    for filename in glob.glob(
        os.path.join(image_folder, "*[!_Detected].png")
    ):  # read pngs in a folder
        # read the image
