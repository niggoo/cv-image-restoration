...
from utils import (
    ...
    upload_images_was,
    create_dummylocation_id_was,
    upload_dummylocation_id_was,
    upload_detectionlabels_was,
)
...
class ServerUpload:
    ...
    async def server_upload(self, UploadQueue, upload_complete_event):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            await upload_dummylocation_id_was(
                session, self._serveradd, self._location_id
            )
            time.sleep(1.0)
                    ...
                    if not UploadQueue.empty():
                        ...
                        if len(Image_IDList):
                            ...
                            IntegralImageList = self._ImageIDList[
                                min(0, len(self._ImageIDList) - 30) : len(
                                    self._ImageIDList
                                )
                                - 1
                            ]
                        ...
                        if len(Labels):
                            await upload_detectionlabels_was(
                                session, self._serveradd, self._location_id, Labels
                            )
...
if __name__ == "__main__":
    ...
    individualImageLocation = os.path.join(
        basedatapath, "..", "data", sitename, "images"
    )
    integralImageLocation = os.path.join(
        basedatapath, "..", "data", sitename, "testresults"
    )
    individualimagelist = [
        x for x in glob.glob(os.path.join(individualImageLocation, "*.png"))
    ]
    integralimagelist = [
        x for x in glob.glob(os.path.join(integralImageLocation, "*.png"))
    ]
    ImageList = []
    ViewMatList = []
