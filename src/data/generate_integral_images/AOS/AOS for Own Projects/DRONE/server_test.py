...
class Dummy:
    ...
    async def dummy_individualimage(self, imagename):
        ...
        image_id = await upload_images(
            self._serveradd, image, generatedviewmatrix, self._location_id, poses=None
        )
    ...
    def dummy_individualimage_was(self, imagename):
        image = cv2.imread(imagename)
        generatedviewmatrix = np.zeros((3, 4))
        image_id = upload_images_was(
            self._serveradd, image, generatedviewmatrix, self._location_id, poses=None
        )
        print("individual image id", image_id)
        return image_id

    async def dummy_integralimages(
        self, renderedimagelist, individualimagelist, detectevent
    ):
        count = 0
    ...
    def dummy_run(self, renderedimagelist, individualimagelist, detectevent):
        self.dummy_integralimages_was(
            renderedimagelist, individualimagelist, detectevent
        )
...
if __name__ == "__main__":
    ...
    separateprocess = multiprocessing.Process(
        name="RenderProcess",
        target=DummyClass.dummy_run,
        args=(
            integralimagelist,
            individualimagelist,
            detectevent,
