...
class DroneFlyingControl:
    ...
    def __init__(
        ...
        area_sides=None,
        ReadfromFile=False,
        maxflighttime=20.0 * 60.0,
        simulate=False,
        FlirAttached=False,
        startlat=0,
        startlon=0,
        out_folder="FlyingControl_results",
        GridAlignedPathPlanning=True,
        PiWayPointRadiusCheck=5.0,
        PlanningAlgo=None,
        Flying_Height=35,
        DroneFlyingSpeed=1,
        FasterDroneFlyingSpeed=3,
        WayPointHoldingTime=5,
        GrabVideoFrames=True,
        ImageSamplingDistance=1.0,
        SimulatedData=None,
        Render=True,
        RenderAfter=1,
        CenterUTMInfo=None,
    ...
    def FlyingControl(
        ...
        """
        self._PlanningAlgo = Planner(
            utm_center=(
                self._CenterUTMInfo[0],
                self._CenterUTMInfo[1],
                self._CenterUTMInfo[2],
                self._CenterUTMInfo[3],
            ),
            area_size=self._area_sides,
            tile_distance=self._GridSideLength,
            prob_map=self._prob_map,
            debug=False,
            vis=None,
            results_folder=self._out_folder,
            gridalignedplanpath=self._GridAlignedPathPlanning,
        )
        self._log = setup_logger(
            "Flying_logger", os.path.join(self._out_folder, "FlyControlInfoLog.log")
        )
        self._gpsframelog = setup_logger(
            "GPSFrame_logger", os.path.join(self._out_folder, "GPSLog.log")
        )
        ...
        try:
            ...
            startEast, startNorth, Block, UTMZONE = utm.from_latlon(
            ...
            MinimumStartingHeigth, Index = FindStartingHeight(
                self._objectmodelpath, StartCenteredEastUTM, StartCenteredNorthUTM
            )
            # CaptureRenderDetectCurrentData.AddStartingHeight(MinimumStartingHeigth)
            ####Todo :- Communicate MinimumStartingHeight to Renderer
            self._log.debug(
                "StartingPoint StartLatitude = %s StartLongitude =%s StartEast = %s StartNorth = %s CenterEast = %s CenterNorth = %s StartCenterEast = %s StartCenterNorth = %s And StartingHeight = %s",
                str(StartLatitude),
                str(StartLongitude),
                str(startEast),
                str(startNorth),
                str(self._CenterEast),
                str(self._CenterNorth),
                str(StartCenteredEastUTM),
                str(StartCenteredNorthUTM),
                str(MinimumStartingHeigth),
            )
            self._gpsframelog.debug("%s %s", str(StartLatitude), str(StartLongitude))
            self._gpsframelog.debug(
                "Time Latitude Longitude Altitude CompassHeading TargetHoldTime(from GPSLog)"
            )
            ...
            while CurrentFlightTime < self._maxflighttime:
                    ...
                    if self._SendEmail:
                        ...
                        server.sendmail(
                            ...
                            CreateText(
                                ...
                                self._subject,
                                self._body,
                                DetectionInfo["DetectedImageName"],
                            ),
                        )
                ...
                if len(next_pts) >= 1:
                    ...
                    for i in range(len(next_pts)):
                        ...
                        self._log.debug(
                            "Sending to Position %s %s %s with Flying Speed %s",
                            str(DestinationLat),
                            str(DestinationLon),
                            str(NextAltitude),
                            str(FlyingSpeed),
                        ...
                        if CompleteRendering:
                            self._gpsframelog.debug(
                                "%s %s %s %s",
                                str(x - 1),
                                str(NoofPointsinPreviousPath),
                                str(NoofPointsChecked),
                                str(0),
                            )
                            CompleteRendering = False
                            NoofPointsinPreviousPath = 0
                        ...
                        while CheckPoint == True:
                            ...
                            if not self._ReadfromFile:
                                ...
                                Latitude = (
                                    ReceivedCurrentDroneData["Latitude"] * 0.0000001
                                )
                                Longitude = (
                                    ReceivedCurrentDroneData["Longitude"] * 0.0000001
                                )
                                Altitude = (
                                    ReceivedCurrentDroneData["BaroAltitude"] / 100
                                )
                                DroneTargetHoldTime = ReceivedCurrentDroneData[
                                    "TargetHoldTime"
                                ]
                                CompassHeading = ReceivedCurrentDroneData[
                                    "CompassHeading"
                                ]
                            else:
                                SimulatedDataGPSInfo = self._SimulatedData[
                                    NoofCheckedWayPoints
                                ]
                                Latitude = SimulatedDataGPSInfo["Latitude"]
                                Longitude = SimulatedDataGPSInfo["Longitude"]
                                Altitude = SimulatedDataGPSInfo["BaroAltitude"]
                                DroneTargetHoldTime = SimulatedDataGPSInfo[
                                    "TargetHoldTime"
                                ]
                                ...
                                DistancetoTarget = 0
                            Distance = haversine(
                                Latitude, Longitude, next_pos[0], next_pos[1]
                            )
                            # print('checking Waypoint')
                            if (
                                DroneTargetHoldTime < self._WayPointHoldingTime
                                and DroneTargetHoldTime > 0.0
                            ):
                            ...
                            if FlyingInfo:
                                if self._adddebugInfo:
                                    self._log.debug(
                                        "Curent Position Considered%s %s %s %s",
                                        str(Latitude),
                                        str(Longitude),
                                        str(Altitude),
                                        str(CompassHeading),
                                ...
                                if not NoofPointsinPreviousPath:
                                    ...
                                    if self._adddebugInfo:
                                        ...
                                        self._log.debug(
                                            "Current Image is on new path AddPreviousImage is %s and AddCurrentImage is  %s",
                                            str(AddPreviousImage),
                                            str(AddCurrentImage),
                                        )

                                else:
                                    CurrentImagetoPreviousSampleDistance = haversine(
                                        Latitude,
                                        Longitude,
                                        PrevSampleLat,
                                        PrevSampleLon,
                                    )
                                    CurrentImagetoPreviousImageDistance = haversine(
                                        Latitude, Longitude, PrevLat, PrevLon
                                    )
                                    if self._adddebugInfo:
                                        self._log.debug(
                                            "distance Calculation %s %s %s %s %s %s",
                                            str(PrevSampleLat),
                                            str(PrevSampleLon),
                                            str(PrevLat),
                                            str(PrevLon),
                                        ...
                                        self._log.debug(
                                            "CurrentImagetoPreviousSampleDistance is %s and CurrentImagetoPreviousImageDistance is  %s and PrevImagetoPrevSampleDistance is %s",
                                            str(CurrentImagetoPreviousSampleDistance),
                                            str(CurrentImagetoPreviousImageDistance),
                                            str(PrevImagetoPrevSampleDistance),
                                        )
                                    if (
                                        CurrentImagetoPreviousSampleDistance
                                        >= self._ImageSamplingDistance
                                        or ReachedWayPoint == True
                                    ):
                                        if (
                                            abs(PrevImagetoPrevSampleDistance - 0.0)
                                            > 0.01
                                        ):
                                            if (
                                                abs(
                                                ...
                                                if self._adddebugInfo:
                                                    self._log.debug(
                                                        "PrevSample Position %s %s",
                                                        str(PrevSampleLat),
                                                        str(PrevSampleLon),
                                                    )
                                                    self._log.debug(
                                                        "Current Position %s %s %s %s %s",
                                                        str(Latitude),
                                                        str(Longitude),
                                                        str(Altitude),
                                                        str(CompassHeading),
                                                    ...
                                                    self._log.debug(
                                                        "Current Image is closer to 1m than previous image AddPreviousImage is %s and AddCurrentImage is  %s",
                                                        str(AddPreviousImage),
                                                        str(AddCurrentImage),
                                                    )
                                            else:
                                                ...
                                                NoofPointsinPreviousPath = (
                                                    NoofPointsinPreviousPath + 1
                                                )
                                                gray = cv2.cvtColor(
                                                    PrevImage, cv2.COLOR_BGR2GRAY
                                                )
                                                ...
                                                PrevImagetoPrevSampleDistance = (
                                                    CurrentImagetoPreviousImageDistance
                                                )
                                                if self._GrabVideoFrames:
                                                    if self._Render:
                                                ...
                                                if self._adddebugInfo:
                                                    ...
                                                    self._log.debug(
                                                        "previous image is closer to 1m AddPreviousImage is %s and AddCurrentImage is  %s",
                                                        str(AddPreviousImage),
                                                        str(AddCurrentImage),
                                                    )
                                                else:
                                                ...
                                                if (
                                                    ...
                                                    if self._adddebugInfo:
                                                        ...
                                                        self._log.debug(
                                                            "current image is 1m apart from previous imageAddPreviousImage is %s and AddCurrentImage is  %s",
                                                            str(AddPreviousImage),
                                                            str(AddCurrentImage),
                                                        )
                                                else:
                                ...
                                if AddCurrentImage == True:
                                    ...
                                    if self._adddebugInfo:
                                        ...
                                        self._log.debug(
                                            "AddPreviousImage is %s and AddCurrentImage is  %s",
                                            str(AddPreviousImage),
                                            str(AddCurrentImage),
                                        )
                                    else:
                        ...
                        if self._adddebugInfo:
                            self._log.debug(
                                "Calculated CurrentFlightTime %s",
                                str(CurrentFlightTime),
                            )
                else:
...
if __name__ == "__main__":
    ...
    InterPolatedGPSReceivedLogFileName = os.path.join(
        basedatapath, "..", "data", sitename, "log", "GPSInterpolatedLog.log"
    )
    (
        InterpolatedLat,
        InterpolatedLon,
    ...
    ) = ReadInterpolatedGPSlogFiles(InterPolatedGPSReceivedLogFileName)
    ...
    FlyingControlClass = DroneFlyingControl(
        sitename=sitename,
        CenterEast=CenterEast,
        CenterNorth=CenterNorth,
        objectmodelpath=ObjModelPath,
    ...
    FlyingControlProcess = multiprocessing.Process(
        name="FlyingControlProcess",
    ...
    for i in range(len(InterpolatedLat)):
        ...
        CurrentDroneInfoDict["Image"] = np.random.randint(
            0, 255, size=(640, 480, 3)
        ).astype(np.uint8)
        ...
        if not RenderingQueue.empty():
            ...
            MainLog.debug(
                "Rendering Info Received %s %s",
                str(RenderingInfo["Latitude"]),
                str(RenderingInfo["Longitude"]),
            )
        time.sleep(0.01)
