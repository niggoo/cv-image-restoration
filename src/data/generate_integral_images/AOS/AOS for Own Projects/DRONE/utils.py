...
def GenerateGPSGridInfo(
    CenterUTMCoordinates,
    DEMCenterUTMInfo,
    GridSideLength,
    AreaSideLength,
    AreaSideWidth,
    ...
    LengthRangePointList = [
        *range(
            -int(math.ceil(NoofUnitsinLength / 2)), int(NoofUnitsinLength / 2) + 1, 1
        )
    ]
    WidthRangePointList = [
        *range(-int(math.ceil(NoofUnitsinWidth / 2)), int(NoofUnitsinWidth / 2) + 1, 1)
    ]
    ...
    GPSCenterPoints = np.zeros(
        (len(LengthRangePointList) - 1, len(WidthRangePointList) - 1, 2)
    )
    ...
    for GPH in range(
        ...
        BaseEast = CenterUTMEast + (
            GPH * GridSideLength * np.cos(np.deg2rad(RotationAngle))
        )
        BaseNorth = CenterUTMNorth + (
            GPH * GridSideLength * np.sin(np.deg2rad(RotationAngle))
        )
        CountH = CountH + 1
        ...
        print(CountCH)
        for GPV in range(
            -int(math.ceil(NoofUnitsinWidth / 2)), int(NoofUnitsinWidth / 2) + 1, 1
        ):
            PointEast = BaseEast + (
                GPV * GridSideLength * np.sin(np.deg2rad(-RotationAngle))
            )
            PointNorth = BaseNorth + (
                GPV * GridSideLength * np.cos(np.deg2rad(-RotationAngle))
            )
            if GPV <= 0:
                CenterPointEast = PointEast + (
                    (GridSideLength / math.sqrt(2))
                    * np.sin(np.deg2rad(-RotationAngle + 45))
                )
                CenterPointNorth = PointNorth + (
                    (GridSideLength / math.sqrt(2))
                    * np.cos(np.deg2rad(-RotationAngle + 45))
                )
            else:
                CenterPointEast = PointEast + (
                    (GridSideLength / math.sqrt(2))
            ...
            Lat, Lon = utm.to_latlon(
                EastPoint, NorthPoint, CenterUTMCoordinates[2], CenterUTMCoordinates[3]
            )
            CenterLat, CenterLon = utm.to_latlon(
                CenterEastPoint,
                CenterNorthPoint,
                CenterUTMCoordinates[2],
                CenterUTMCoordinates[3],
            )
...
def ReadandUpdateWaypointList(NoofPoints, Rwp):
    ...
    for x in range(NoofPoints):
        ...
        if y[0] == "True":
            ...
            StopFlag = False
        print(
            "Lat , Long ,  Alt, CalibrationWaypoint,StopFlag ",
            ReadLatitude,
            ReadLongitude,
            ReadAltitude,
            CalibrationWayPoint,
        ...
        WayPointlist.append(
            WayPointInfoCheck(
                ReadLatitude, ReadLongitude, ReadAltitude, CalibrationWayPoint, StopFlag
            )
        )
...
def CreateJsonPoseFile(
    ...
    for i in range(0, len(EastCenteredList)):
        ...
        M = createviewmateuler(
            np.array([math.radians(CompassRad), 0, 0]),
            np.array([EastCenteredList[i], NorthCenteredList[i], Altitude]),
        )
    ...
    with open(DemFileName) as DemFile:
        ...
        while True:
            ...
            if "v" in CheckVertex:
                ...
                UTMDistance = math.sqrt(
                    (
                        (StartCenteredEastUTM - VertexEast)
                        * (StartCenteredEastUTM - VertexEast)
                    )
                    + (
                        (StartCenteredNorthUTM - VertexNorth)
                        * (StartCenteredNorthUTM - VertexNorth)
                    )
                )
...
def determine_Location_inView(demArray, XCoord, YCoord):
    X = np.sqrt(
        np.square(demArray[:, :, 0] - XCoord) + np.square(demArray[:, :, 1] - YCoord)
    )
...
async def upload_png_file(session, serveraddres, image_ref, file):
    ...
    with open(file, "rb") as f:
        async with session.request(
            ...
            return resp.headers[
                "Location"
            ]  # this one is needed for linking with the metadata
...
async def upload_file(session, resource, file, mime=None):
    ...
    with open(file, "rb") as f:
        async with session.request("put", resource, data=f, headers=headers) as resp:
            await resp.read()
...
async def upload_json(session, resource, data):
    # print('url',resource)
    async with session.request(
        "post", resource, data=data, headers={"content-type": "application/json"}
    ) as resp:
        await resp.read()
...
async def download_file(serveraddress, resources, local_file, remote_file):
    async with aiohttp.ClientSession() as session:
        ...
        async with session.request(
            "get",
            os.path.join(serveraddress, resources, remote_file).replace("\\", "/"),
            params=None,
        ) as resp:
...
async def create_dummylocation_id(serveraddress, locationid):
    async with aiohttp.ClientSession() as session:
        ...
        async with session.request(
            "put",
            serveraddress + "/drones/drone1",
            data=json.dumps(
                {
                    "camera": {
                        "fov": {"Deg": 50.815436217896945},
                        "near": 0.1,
                        "far": 100.0,
                        "aspect": 1.0,
                        "resolution": [512, 512],
        ...
        async with session.request(
            "put",
            serveraddress + "/locations/" + location_id,
            data=json.dumps(
                {
...
async def upload_images(
    serveraddress, undstortedimage, generatedviewmatrix, locationid, poses=None
):
    async with aiohttp.ClientSession() as session:
        ...
        image_ref = await create_image_meta(
            session, serveraddress + resources, json.dumps(data)
        )
        is_success, img_encoded = cv2.imencode(".png", undstortedimage)
        image_location = await upload_png_file_data(
            session, serveraddress + image_ref, img_encoded.tobytes()
        )
        return image_ref


async def upload_images_was(
    session, serveraddress, undstortedimage, generatedviewmatrix, locationid, poses=None
):
    if poses is not None:
        resources = "/integrals"
        data = {
