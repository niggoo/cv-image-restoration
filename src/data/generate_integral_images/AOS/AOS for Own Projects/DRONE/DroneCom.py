...
class CurrentDroneInfo(ct.Structure):
    _fields_ = [
        ...
        ("Longitude", ct.c_int),
        ("Altitude", ct.c_int),
        ("BaroAltitude", ct.c_int),
        ("CompassHeading", ct.c_int),
        ("AngleNick", ct.c_int),
        ("AngleRoll", ct.c_int),
        ("WaypointIndex", ct.c_int),
        ("WayPointNumber", ct.c_int),
        ("DistanceToTarget", ct.c_int),
        ("TargetHoldTime", ct.c_int),
    ]
...
def ReadGPSReceivedLogFiles(FileName):
    ...
    (
        ...
        DistanceToTarget,
        DroneHoldTime,
    ) = np.genfromtxt(FileName, skip_header=0, unpack=True, delimiter=" ")
...
class DroneCommunication:
    ...
    _so_file = os.path.join(
        Path(__file__).resolve().parent, "dronecommunication.so"
    ...
    def __init__(
        ...
        so_file="/home/pi/PyCLFR_AutoDrone/glesLFR/src/droneCommunication_SI_ZeroTol.so",
    ):
    ...
    def DroneInfo(
        ...
        self._log = setup_logger(
            "GPSReceived_logger", os.path.join(self._out_folder, "GPSReceivedLog.log")
        )
        self._queueLog = setup_logger(
            "GPSInterpolated_logger",
            os.path.join(self._out_folder, "GPSInterpolatedLog.log"),
        )
        self._sentwaypointlog = setup_logger(
            "SentWayPoint_logger", os.path.join(self._out_folder, "SentWaypoint.log")
        )
        try:
            if not self._simulation:
                self.DroneInit()
            ...
            while not DroneProcessEvent.is_set():
                if not SendWayPointInfoQueue.empty():
                    ...
                    self._sentwaypointlog.debug(
                        "Upload and Send Drone to Waypoint = %s to Lat = %s, Lon = %s, Alt = %s and Speed = %s",
                        str(Index),
                        str(LatToSend),
                        str(LongtoSend),
                        str(AlttoSend),
                    ...
                    if not self._simulation:
                        self._DroneCommunication.sendWayPoint(
                            ct.c_int(LatToSend),
                            ct.c_int(LongtoSend),
                            ct.c_int(AlttoSend),
                            ct.c_int(Index),
                            ct.c_int(SpeedtoSend),
                        )
                    # print('SendWayPointInfoQueue Size', SendWayPointInfoQueue.qsize())
                else:
                    if not self._simulation:
                        self._DroneCommunication.receiveDroneInfo.restype = (
                            CurrentDroneInfo
                        )
                    ...
                    if streamInfo != -1:
                        ...
                        if Index >= 10:
                            if self._interpolation:
                                if (
                                    abs(DroneInfoData.Latitude - self._PreviousLatitude)
                                    > 0.01
                                ) or (
                                    abs(
                                        DroneInfoData.Longitude
                                        - self._PreviousLongitude
                                    )
                                    > 0.01
                                ):
                                    GetFramesEvent.set()
                                    if Index == 15:
                                        self._log.debug(
                                            "%s %s %s %s %s %s New WayPoint",
                                            str(DroneInfoData.Latitude),
                                            str(DroneInfoData.Longitude),
                                            str(DroneInfoData.BaroAltitude),
                                            str(self._PreviousCompassHeading),
                                            str(DroneInfoData.TargetHoldTime),
                                            str(CurrentGPSReceivedtime),
                                        )
                                        self._PreviousDroneTargetHoldTime = (
                                            DroneInfoData.TargetHoldTime
                                        )
                                    elif Index == 16:
                                        self._log.debug(
                                            "%s %s %s %s %s %s New WayPoint",
                                            ...
                                            str(DroneInfoData.Longitude),
                                            str(DroneInfoData.BaroAltitude),
                                            str(DroneInfoData.CompassHeading),
                                            str(self._PreviousDroneTargetHoldTime),
                                            str(CurrentGPSReceivedtime),
                                        )
                                        self._PreviousCompassHeading = (
                                            DroneInfoData.CompassHeading
                                        )
                                    else:
                                        self._log.debug(
                                            "%s %s %s %s %s %s New WayPoint",
                        ...
                        if not RecordEvent.is_set():
                            ...
                            if not self._interpolation:
                                if self._FlirAttached and not self._addsynthethicimage:
                                    (
                                        FrameGrabbingSuccessFlag,
                                        Frame,
                                    ) = FlirVideoSource.read()
                                else:
                                    Frame = np.random.randint(
                                        0,
                                        255,
                                        size=(
                                            self._synthethic_image_res_y,
                                            self._synthethic_image_res_x,
                                            3,
                                        ),
                                    ).astype(np.uint8)
                                ...
                                CurrentDroneInfoDict[
                                    "Latitude"
                                ] = self._PreviousLatitude
                                CurrentDroneInfoDict[
                                    "Longitude"
                                ] = self._PreviousLongitude
                                CurrentDroneInfoDict[
                                    "Altitude"
                                ] = self._PreviousAltitude
                                CurrentDroneInfoDict[
                                    "BaroAltitude"
                                ] = self._PreviousBaroAltitude
                                CurrentDroneInfoDict[
                                    "TargetHoldTime"
                                ] = self._PreviousDroneTargetHoldTime
                                CurrentDroneInfoDict[
                                ...
                                if not FrameQueue.empty():
                                    ...
                                    if len(self._GPSTimeList) > 1:
                                        ...
                                        self._queueLog.debug(
                                            "No of Frames %s %s %s %s %s",
                                            str(len(FrameTimeList)),
                                            str(len(self._CurrentFrameTimeList)),
                                            str(FrameTimeList[0]),
                                            str(FrameTimeList[-1]),
                                            str(CurrentGPSReceivedtime),
                                        )
                                        ...
                                        for i in range(len(FrameTimeList)):
                                            if FrameTimeList[i] > self._GPSTimeList[-1]:
                                                if self._extrapolate:
                                                    self._CurrentFrameTimeList.append(
                                                        FrameTimeList[i]
                                                    )
                                                    self._CurrentFrameList.append(
                                                        FrameList[i]
                                                    )
                                                else:
                                                    self._NextFrameTimeList.append(
                                                        FrameTimeList[i]
                                                    )
                                                    self._NextFrameList.append(
                                                        FrameList[i]
                                                    )
                                            else:
                                                self._CurrentFrameTimeList.append(
                                        ...
                                        if not self._extrapolate:
                                            FrameInterpolatedLatList = np.interp(
                                                self._CurrentFrameTimeList,
                                                self._GPSTimeList,
                                                self._GPSLatList,
                                            )
                                            FrameInterpolatedLonList = np.interp(
                                                self._CurrentFrameTimeList,
                                                self._GPSTimeList,
                                                self._GPSLonList,
                                            )
                                            FrameLatInterpolatedAltList = np.interp(
                                                self._CurrentFrameTimeList,
                                                self._GPSTimeList,
                                                self._GPSAltList,
                                            )
                                            FrameLatInterpolatedCompList = np.interp(
                                                self._CurrentFrameTimeList,
                                                self._GPSTimeList,
                                                self._GPSCompList,
                                            )
                                        else:
                                            InterpolatedLatFunction = (
                                        ...
                                        for i in range(len(self._CurrentFrameList)):
                                            ...
                                            CurrentDroneInfoDict[
                                                "Latitude"
                                            ] = FrameInterpolatedLatList[i]
                                            CurrentDroneInfoDict[
                                                "Longitude"
                                            ] = FrameInterpolatedLonList[i]
                                            CurrentDroneInfoDict[
                                                "Altitude"
                                            ] = self._PreviousAltitude
                                            CurrentDroneInfoDict[
                                                "BaroAltitude"
                                            ] = FrameLatInterpolatedAltList[i]
                                            CurrentDroneInfoDict[
                                                "TargetHoldTime"
                                            ] = self._PreviousDroneTargetHoldTime
                                            CurrentDroneInfoDict[
                                                "CompassHeading"
                                            ] = FrameLatInterpolatedCompList[i]
                                            CurrentDroneInfoDict[
                                                "Image"
                                            ] = self._CurrentFrameList[i]
                                            CurrentGPSInfoQueue.put(
                                                CurrentDroneInfoDict
                                            )
                                            self._queueLog.debug(
                                                "%s %s %s %s %s %s",
                                                str(CurrentDroneInfoDict["Latitude"]),
                                                str(CurrentDroneInfoDict["Longitude"]),
                                                str(
                                                    CurrentDroneInfoDict["BaroAltitude"]
                                                ),
                                                str(
                                                    CurrentDroneInfoDict[
                                                        "CompassHeading"
                                                ...
                                                str(self._CurrentFrameTimeList[i]),
                                            )
                                        FrameInterpolatedLatList = []
                                        FrameInterpolatedLonList = []
                                        FrameLatInterpolatedAltList = []
                                        ...
                                        self._CurrentFrameList = []
                                        if not self._extrapolate:
                                            self._CurrentFrameTimeList.extend(
                                                self._NextFrameTimeList
                                            )
                                            self._CurrentFrameList.extend(
                                                self._NextFrameList
                                            )
                                        self._NextFrameTimeList = []
                                        self._NextFrameList = []
...
if __name__ == "__main__":
    ...
    GPSReceivedLogFile = os.path.join(
        basedatapath, "..", "data", sitename, "log", "GPSReceivedLog.log"
    )
    GPSlogFileInfo = ReadGPSReceivedLogFiles(GPSReceivedLogFile)

    CameraClass = CameraControl(
        FlirAttached=False, AddsynthethicImage=True, out_folder=out_folder
    )

    DroneComm = DroneCommunication(
        simulate=True,
    ...
    processes = []
    CameraAcquireFramesProcess = multiprocessing.Process(
        name="CameraAcquireFramesProcess",
    ...
    while GPSCount <= 1000 or not CurrentGPSInfoQueue.empty():
        ...
        if not CurrentGPSInfoQueue.empty():
            ...
            MainLog.debug(
                "%s %s %s %s %s",
                str(ReceivedCurrentDroneData["Latitude"]),
                str(ReceivedCurrentDroneData["Longitude"]),
                str(ReceivedCurrentDroneData["BaroAltitude"]),
                str(ReceivedCurrentDroneData["CompassHeading"]),
                str(ReceivedCurrentDroneData["TargetHoldTime"]),
            )
    ...
    for process in processes:
        process.join(5)
    print(
        "CameraAcquireFramesProcess.is_alive()", CameraAcquireFramesProcess.is_alive()
    )
