...
class InitializationClass:
    ...
    def __init__(
        ...
        StartLatitudeGlobal=0.0,
        dc=10,
        StartLongitudeGlobal=0.0,
        FasterDroneFlyingSpeed=10,
        WayPointHoldingTime=5,
        PiWayPointRadiusCheck=5.0,
        TimeDelayForFlirConnection=7.0,
        pwmPin=18,
        LowerThreshold=0.05,
        UpperThreshold=0.10,
        Render=True,
        ...
        _NormalizedDistance=30,
        AlwaysRenderSeparetly=True,
        SimulateonCPU=False,
        GridAlignedPathPlanning=True,
        ContinuousIntegration=True,
        ContinuousIntegrationAfterNoofImages=10,
        DetectWithPiImages=False,
        SendEmail=False,
        ...
        self._ContinuousIntegrationAfterNoofImages = (
            ContinuousIntegrationAfterNoofImages
        )
        ...
        self._device = (
            "MYRIAD" if IntelStickAttached else "CPU"
        )  # "CPU" or "MYRIAD"(compute stick)
        ...
        if self._DetectWithPiImages:
            self._PiImagesFolder = os.path.join(
                self._basedatapath, "..", "data", sitename, "PiRenderedResults"
            )
        self._Download_Dest = os.path.join(
            self._basedatapath, "..", "data", sitename, "Image"
        )
        self._WritePosesPath = os.path.join(
            self._basedatapath, "..", "data", sitename, "FlightPoses"
        )
        self._SavProjPath = os.path.join(
            self._basedatapath, "..", "data", sitename, "Projections"
        )
        self._ObjModelPath = os.path.join(
            self._basedatapath, "..", "data", sitename, "DEM", "dem.obj"
        )
        self._ObjModelImagePath = os.path.join(
            self._basedatapath, "..", "data", sitename, "DEM", "dem.png"
        )
        self._LFRPath = os.path.join(self._basedatapath, "..", "data", sitename, "DEM")
        self._DemInfoJSOn = os.path.join(
            self._basedatapath, "..", "data", sitename, "DEM", "dem_info.json"
        )
...
if __name__ == "__main__":
    ...
    location_ref = (
        "test_flight_server_upload"  # Find Way to Get locationref from the server
    )
    ...
    InitializedValuesClass = InitializationClass(
        sitename=sitename,
        area_sides=(90, 90),
        ReadfromFile=False,
        DroneAttached=True,
    ...
    CurrentGPSInfoQueue = multiprocessing.Queue(
        maxsize=200
    )  # queue which stores gps tagged frames.
    ...
    SendWayPointInfoQueue = multiprocessing.Queue(
        maxsize=20
    )  # waypoint information queue.get() returns a dictionary as:
    ...
    RenderingQueue = multiprocessing.Queue(
        maxsize=200
    )  # queue with geotagged frames with ~1m spacing
    ...
    FrameQueue = multiprocessing.Queue(
        maxsize=200
    )  # a queue element is a dictionary of the form
    #   {   'Frames': [img1, img2, ...],
    ...
    )  # enabling this event (.set) stops the DroneCommunication process terminally (only do once)
    FlyingProcessEvent = (
        multiprocessing.Event()
    )  # if enabled (signaled from the DroneFlyingControl) the last waypoint has been reached
    RenderingProcessEvent = (
        multiprocessing.Event()
    )  # enabling this event (.set) stops the Renderer process terminally (only do once)
    CameraProcessEvent = (
        multiprocessing.Event()
    )  # enabling this event (.set) stops the camera process terminally (only do once)
    GetFramesEvent = (
        multiprocessing.Event()
    )  # enable if you want to retrieve recorded frames
    RecordEvent = (
        multiprocessing.Event()
    )  # enable if you want to record information while flying
    upload_complete_event = multiprocessing.Event()

    if InitializedValuesClass._ReadfromFile:
        GPSReceivedLogFile = os.path.join(
            InitializedValuesClass._basedatapath,
            "FlightResults",
            InitializedValuesClass._sitename,
            "GPSReceivedLog.log",
        )
    ...
    CameraClass = CameraControl(
        FlirAttached=InitializedValuesClass._FlirAttached,
        AddsynthethicImage=False,
    ...
    DroneCommunicationClass = DroneCommunication(
        simulate=False,
        GPSLog=GPSlogFileInfo,
        interpolation=True,
        extrapolate=False,
        AddSynthethicImage=False,
        FlirAttached=InitializedValuesClass._FlirAttached,
    ...
    FlyingControlClass = DroneFlyingControl(
        sitename=InitializedValuesClass._sitename,
        CenterEast=InitializedValuesClass._CenterEast,
        CenterNorth=InitializedValuesClass._CenterNorth,
        objectmodelpath=InitializedValuesClass._ObjModelPath,
        basedatapath=InitializedValuesClass._basedatapath,
        Render=True,
        FlirAttached=InitializedValuesClass._FlirAttached,
        Flying_Height=InitializedValuesClass._Flying_Height,
        DroneFlyingSpeed=InitializedValuesClass._DroneFlyingSpeed,
        RenderAfter=2,
        CenterUTMInfo=InitializedValuesClass._CenterUTMInfo,
        out_folder=os.path.join(
            InitializedValuesClass._basedatapath,
            "..",
            "data",
        ...
        area_sides=InitializedValuesClass._area_sides,
        GridSideLength=InitializedValuesClass._GridSideLength,
        GridAlignedPathPlanning=InitializedValuesClass._GridAlignedPathPlanning,
        prob_map=InitializedValuesClass._prob_map,
        UpdatePathPlanningflag=InitializedValuesClass._UpdatePathPlanning,
        adddebugInfo=True,
    )
    ...
    RendererClass = Renderer(
        CenterUTMInfo=InitializedValuesClass._CenterUTMInfo,
        ObjModelPath=InitializedValuesClass._ObjModelPath,
        Detect=True,
        ObjModelImagePath=InitializedValuesClass._ObjModelImagePath,
        basedatapath=InitializedValuesClass._basedatapath,
        sitename=InitializedValuesClass._sitename,
        ...
        FieldofView=InitializedValuesClass._FieldofView,
        device="MYRIAD",
        adddebuginfo=True,
        uploadserver=True,
    ...
    RenderProcess = multiprocessing.Process(
        ...
        args=(RenderingQueue, DetectionInfoQueue, uploadqueue, RenderingProcessEvent),
    )
    processes.append(RenderProcess)
    RenderProcess.start()
    time.sleep(2)

    DroneCommunicationProcess = multiprocessing.Process(
        name="DroneCommunicationProcess",
    ...
    CameraFrameAcquireProcess = multiprocessing.Process(
        name="CameraFrameAcquireProcess",
