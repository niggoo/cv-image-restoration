...
def compute_end_point(start_pos, next_pos, max_path_length):
    start_East, start_North, start_Block, start_UTMZone = utm.from_latlon(
        start_pos[0], start_pos[1]
    )
    next_East, next_North, next_Block, next_UTMZone = utm.from_latlon(
    ...
    end_pos_utm = (
        start_East + (((next_East - start_East) / nextposdistance) * max_path_length),
        start_North
        + (((next_North - start_North) / nextposdistance) * max_path_length),
    )
    print("start and end utm", (start_East, start_North), end_pos_utm)
    end_pos_Lat, end_pos_Lon = utm.to_latlon(
        end_pos_utm[0], end_pos_utm[1], start_Block, next_UTMZone
    )
    ...
    print(
        ...
        start_pos,
        end_pos,
        compute_distance_bw_2gpsPoints(start_pos, end_pos),
    )
...
def compute_distance_bw_2gpsPoints(Prev_Pos, Next_Pos):
    Prev_East, Prev_North, Prev_Block, Prev_UTMZone = utm.from_latlon(
...
def findwaypointsinfo(Row, Col, gps_grid_waypoints, prev_waypoint, prev_North_dir):
    ...
    FlyingNorthDir = None
    for i in range(4):
        Distance = compute_distance_bw_2gpsPoints(
            prev_waypoint,
            (gps_grid_waypoints[Row, Col, i, 0], gps_grid_waypoints[Row, Col, i, 1]),
        )
    ...
    if MinDistance < 1:
        if MinIndex < 2:
            Nextpos.append(
                (
                    gps_grid_waypoints[Row, Col, MinIndex + 2, 0],
                    gps_grid_waypoints[Row, Col, MinIndex + 2, 1],
                )
            )
        else:
            Nextpos.append(
        ...
        if MinIndex < 2:
            Nextpos.append(
                (
                    gps_grid_waypoints[Row, Col, MinIndex, 0],
                    gps_grid_waypoints[Row, Col, MinIndex, 1],
                )
            )
            Flag.append(False)
            Nextpos.append(
...
def compute_Aexp(Cov, P, GPSCenterPoints, current_pos, NormalizedDistance):
    ...
    CurrentPosEast, CurrentPosNorth, Block, UtmZone = utm.from_latlon(
    ...
    for i in range(Cov.shape[0]):
        for j in range(Cov.shape[1]):
            ...
            CellEast, CellNorth, Block, UTMzone = utm.from_latlon(
                GPSCenterPoints[i, j, 0], GPSCenterPoints[i, j, 1]
            )
            if NormalizedDistance <= 0:
                ...
                C = math.exp(
                    (math.sqrt((CellEast - si) ** 2 + (CellNorth - sj) ** 2))
                    / NormalizedDistance
                )
...
def compute_Navg(
    Cov, P, GPSCenterPoints, current_pos, selected_cells, Ncycle, NormalizedDistance
):
    assert Cov.shape == P.shape
    ...
    for i_c in range(len(selected_cells[0])):
        ...
        for ii in range(i2, i3):
            for jj in range(j2, j3):
                ...
                CellEast, CellNorth, Block, UTMzone = utm.from_latlon(
                    GPSCenterPoints[ii, jj, 0], GPSCenterPoints[ii, jj, 1]
                )
                if NormalizedDistance <= 0:
                    # ignore the distance
                    C = 1.0
...
class Planner:
    ...
    def __init__(
        ...
        conf_threshold=0.05,
        debug=False,
        results_folder="planning_results",
        vis=None,
        ...
        self._log = setup_logger(
            "Planning_logger", os.path.join(results_folder, "PlanningLog.log")
        )
        self._detection_log = setup_logger(
            "Detections_logger", os.path.join(results_folder, "DetectionsLog.log")
        )
        ...
        (
            self._utm_grid,
            self._gps_grid,
            self._utm_corner_grid,
            self._gps_corner_grid,
            self._utm_grid_waypoints,
            self._gps_grid_waypoints,
        ) = compute_grid(utm_center, area_size, tile_distance)
        ...
        if self._debug:
            ...
            if self._vis is not None:
                ...
                self._vis.draw_points(
                    gps_corners, marker="+", color="green"
                )  # corners of area
                self._vis.draw_points(
                    zip(
                        self._gps_grid[:, :, 0].flatten(),
                        self._gps_grid[:, :, 1].flatten(),
                    ),
                    marker="s",
                    color="white",
                )  # grid centers
                self._vis.draw_grid(
                    zip(
                        self._gps_corner_grid[:, :, 0].flatten(),
                        self._gps_corner_grid[:, :, 1].flatten(),
                    ),
                    color="white",
                )  # grid corners
                ...
                self._vis.draw_points(
                    zip(
                        self._gps_grid_waypoints[:, :, :, 0].flatten(),
                        self._gps_grid_waypoints[:, :, :, 1].flatten(),
                    ),
                    marker="+",
                    color="white",
                )  # grid centers
                ...
                plt.plot(
                    [c[0] for c in gps_corners],
                    [c[1] for c in gps_corners],
                    "+",
                    c="dimgray",
                    lw=2,
                )
    ...
    def planpoints(self, current_gps_pos):
        with self._lock:
            ...
            self._log.debug(
                "planning {} path for current point {:f}, {:f}".format(
                    "coarse" if not self.useFineSampling() else "fine",
                    current_gps_pos[0],
                    current_gps_pos[1],
                )
            )
            if not self._grid_aligned_pathplan:
                if self.useFineSampling():
                    next_pts, cov_fine_index = self.planpath(
                        current_gps_pos,
                        self._fine_gps_grid,
                        self._fine_coverage_map,
                        self._fine_prob_map,
                        norm_distance=0,
                        num_points=1,
                    )
                    self._log.debug(
                        "fine coverage map: " + str(self._fine_coverage_map)
                    )
                else:
                    next_pts, cov_index = self.planpath(
                        current_gps_pos,
                        self._gps_grid,
                        self._coverage_map,
                        self._prob_map,
                ...
                if self.useFineSampling():
                    (
                        ...
                        self._prev_waypoint,
                        self._prev_flying_north,
                    ) = self.gridalignedplanpath(
                        current_gps_pos,
                        self._prev_waypoint,
                        self._prev_flying_north,
                        self._prev_index,
                        self._fine_gps_grid,
                        self._fine_gps_grid_waypoints,
                        self._fine_coverage_map,
                        self._fine_prob_map,
                        norm_distance=0,
                        num_points=1,
            ...
            self._log.debug(
                "planned {} points for line {}:".format(
                    len(next_pts), self._count_planpoints
                )
                + str(next_pts)
            )
            if self._debug and len(next_pts) > 0:
                ...
                if self._vis is not None:
                    self._vis.draw_points(
                        [current_gps_pos], marker="x", color=color
                    )  # current
                    for i in range(len(next_pts)):
                        ...
                        if planned_gps_points_flag[i]:  # waypoint with recording
                            self._vis.draw_lines(
                                zip(
                                    [p_pt[0], next_pts[i][0]], [p_pt[1], next_pts[i][1]]
                                ),
                                linestyle="--",
                                color=color,
                            )
                    ...
                    self._vis.draw_points(
                        zip([c[0] for c in next_pts], [c[1] for c in next_pts]),
                        marker="+",
                        color=color,
                    )  # planned positions
                    ...
                    self._vis.save(
                        (
                            os.path.join(
                                self._out_folder, "vis_planpoints_{}.png"
                            ).format(self._count_planpoints)
                        )
                    )
                else:
                    plt.plot(
                        ...
                        current_gps_pos[1],
                        "x",
                        c="b" if self.useFineSampling() else "g",
                        lw=2,
                    )
                    plt.plot(
                        [c[0] for c in pts],
                    ...
                    plt.plot(
                        [c[0] for c in next_pts],
                        [c[1] for c in next_pts],
                        "P",
                        c="b" if self.useFineSampling() else "g",
                        lw=2,
                        markersize=10,
                    ...
                    plt.savefig(
                        os.path.join(self._out_folder, "planpoints_{}.png").format(
                            self._count_planpoints
                        )
                    )
            ...
            if self._grid_aligned_pathplan:
                ...
                assert len(next_pts) == len(
                    planned_gps_points_flag
                ), "planpoints: length of points does not match the length of the recording flags!"
    ...
    def planpath(
        ...
        while not flight_finished:
            A = compute_Aexp(
                coverage_map, prob_map, gps_grid, current_gps_pos, norm_distance
            )
            # find the index of maximum A
            max_pos = np.where(A == np.amax(A))
            if len(max_pos[0]) == 1:
                next_pos = (
                    gps_grid[max_pos[0][0], max_pos[1][0], 0],
                    gps_grid[max_pos[0][0], max_pos[1][0], 1],
                )
            ...
            if norm_distance <= 0.0:
                ...
                planned_path_length = (
                    planned_path_length
                    + compute_distance_bw_2gpsPoints(current_gps_pos, next_pos)
                )
                print("planned_path_length", planned_path_length)
                # For Single 30m Line
                if num_points <= 1:
                    end_pos = compute_end_point(
                        current_gps_pos, next_pos, max_path_length
                    )
                    # planned_gps_points.append(end_pos)
                    planned_gps_points[-1] = end_pos
                    planned_path_length = (
                        planned_path_length
                        + compute_distance_bw_2gpsPoints(next_pos, end_pos)
                    )
                else:
                    if planned_path_length >= max_path_length:
                        end_pos = compute_end_point(
                            current_gps_pos,
                            next_pos,
                            (max_path_length - prev_path_length),
                        )
                        # planned_gps_points.append(end_pos)
                        planned_gps_points[-1] = end_pos
                        planned_path_length = (
            ...
            flight_finished = (
                np.all(coverage_map == 0)
                or len(planned_gps_points) >= num_points
                or planned_path_length >= max_path_length
            )  # compute distance with gps_points and check
        return planned_gps_points, last_index

    @staticmethod
    def gridalignedplanpath(
        current_gps_pos,
        prev_waypoint,
        prev_flying_north,
        prev_index,
        ...
        while not flight_finished:
            if norm_distance <= 0.0:
                ...
                (
                    ...
                    0, 0, gps_grid_waypoints, prev_waypoint, prev_north_dir
                )
                last_index = prev_index
            else:
                prev_north_dir = None
                A = compute_Aexp(
                    coverage_map, prob_map, gps_grid, prev_waypoint, norm_distance
                )
                # find the index of maximum A
                max_pos = np.where(A == np.amax(A))
                if len(max_pos[0]) == 1:
            ...
            # check if we are done
            flight_finished = (
                np.all(coverage_map == 0) or len(planned_gps_points) >= num_points
            )  # compute distance with gps_points and check
    ...
    def update(
        self, line_ct_pt_gps, detections
    ):  # FORMAT  of        detections = [{'gps': prev_pt, 'conf': 0.5}, {'gps': next_pt, 'conf': 0.1, }]
        with self._lock:
            self._count_update += 1
            self._log.debug(
                "update for camera {:f}, {:f} with {} detections".format(
                    line_ct_pt_gps[0], line_ct_pt_gps[1], len(detections)
                )
            )
            if self._prev_coverage_index is not None:
                ...
                assert (
                    True
                ), "This should not happen! You called update before planpoints!"
            ...
            for det in detections:
                if det["conf"] >= self._conf_threshold:
                    gps_loc = det["gps"]
                    det_east, det_north, det_block, det_utmzone = utm.from_latlon(
                        gps_loc[0], gps_loc[1]
                    )
                    det_center_east.append(det_east)
                    det_center_north.append(det_north)
                    self._log.debug(
                        "detections in index ({}, {}) of probability map with {:f} confidence at ({}, {}).".format(
                            closest_index[0],
                            closest_index[1],
                            det["conf"],
                            gps_loc[0],
            ...
            if len(confs) > 0:
                ...
                averagegpslat, averagegpslon = utm.to_latlon(
                    average_det_east, average_det_north, det_block, det_utmzone
                )
            ...
            self._prob_map[
                closest_index[0], closest_index[1]
            ] = max_conf  # update probability map!
            self._log.debug(
                "updating index ({}, {}) of probability map with {:f} confidence.".format(
                    closest_index[0], closest_index[1], max_conf
                )
            )
            self._log.debug("probability map: " + str(self._prob_map))
            if len(confs) > 0:
                self._log.debug(
                    "average_det_loc: ({}, {}) ".format(averagegpslat, averagegpslon)
                )
            ...
            if self._debug:
                if self._vis is not None:
                    self._vis.draw_points(
                        [line_ct_pt_gps], marker="^", color="k"
                    )  # current camera
                    self._vis.save(
                        os.path.join(
                            self._out_folder,
                            "vis_update_{}.png".format(self._count_update),
                        )
                    )
                else:
                    plt.plot(
                    ...
                    plt.savefig(
                        os.path.join(
                            self._out_folder, "update_{}.png".format(self._count_update)
                        )
                    )

    def startFineSampling(self, ct_pt_gps, detections):
        self._log.debug(
            "starting fine sampling for center point {:f}, {:f} with {} detections".format(
                ct_pt_gps[0], ct_pt_gps[1], len(detections)
            )
        )
        ...
        if not self._grid_aligned_pathplan:
            (
                self._fine_utm_grid,
                self._fine_gps_grid,
                self._fine_utm_corner_grid,
                self._fine_gps_corner_grid,
                self._fine_utm_grid_waypoints,
                self._fine_gps_grid_waypoints,
            ) = compute_grid(
                utm_center, (self._tile_size, self._tile_size), self._fine_tile_size
            )
        else:
            (
                self._fine_utm_grid,
                ...
                utm_center, (self._tile_size, self._tile_size), self._tile_size
            )

        self._fine_coverage_map = np.zeros(
            self._fine_utm_grid.shape[0:2]
        )  # init with empty
        self._fine_prob_map = np.zeros(
            self._fine_utm_grid.shape[0:2]
        )  # init with empty
        ...
        for det in detections:
            ...
            if True:
                ...
                if self._fine_prob_map[idx[0], idx[1]] < det["conf"]:
                    self._fine_prob_map[idx[0], idx[1]] = det[
                        "conf"
                    ]  # add probability from detection

                if self._debug:
                    if self._vis is not None:
                        self._vis.draw_points(
                            [det["gps"]], marker="*", color="m"
                        )  # show detections!
                    else:
                        plt.plot(
                            det["gps"][0],
...
def compute_corners(utm_center, area_size=(90, 90)):
    ...
    for i in range(len(x)):
        utm_corners.append(
            (
                utm_center[0] + x[i] * hsize[0],
                utm_center[1] + y[i] * hsize[1],
                utm_center[2],
                utm_center[3],
            )
        )
...
def compute_grid(utm_center, area_size, tile_size):
    ...
    utm_grid = np.stack(
        (fes(xnew, ynew) + offset[0], fns(xnew, ynew) + offset[1]), axis=stackaxis
    ).reshape(len(ynew), len(xnew), 2)
    print(utm_grid.shape)
    print(utm_grid)
    gps_grid = utm.to_latlon(
        utm_grid[:, :, 0].flatten(),
        utm_grid[:, :, 1].flatten(),
        utm_center[2],
        utm_center[3],
    ...
    gps_grid = np.stack(
        (
            np.reshape(gps_grid[0], utm_grid.shape[0:2]),
            np.reshape(gps_grid[1], utm_grid.shape[0:2]),
        ),
        axis=2,
    )
    ...
    utm_corner_grid = np.stack(
        (fes(xcornerpoints, ycornerpoints), fns(xcornerpoints, ycornerpoints)), axis=2
    ).reshape(len(ycornerpoints), len(xcornerpoints), 2)
    gps_corner_grid = utm.to_latlon(
        utm_corner_grid[:, :, 0].flatten(),
        utm_corner_grid[:, :, 1].flatten(),
        utm_center[2],
        utm_center[3],
    )
    # print(gps_corner_grid)
    # print(gps_grid)
    gps_corner_grid = np.stack(
        (
            np.reshape(gps_corner_grid[0], utm_corner_grid.shape[0:2]),
            np.reshape(gps_corner_grid[1], utm_corner_grid.shape[0:2]),
        ),
        axis=2,
    )

    utm_cell_vertical_center_grid = np.stack(
        (fes(xnew, ycornerpoints) + offset[0], fns(xnew, ycornerpoints)), axis=2
    ).reshape(len(ycornerpoints), len(xnew), 2)
    gps_cell_vertical_center_grid = utm.to_latlon(
        utm_cell_vertical_center_grid[:, :, 0].flatten(),
        utm_cell_vertical_center_grid[:, :, 1].flatten(),
        utm_center[2],
        utm_center[3],
    )
    gps_cell_vertical_center_grid = np.stack(
        (
            np.reshape(
                gps_cell_vertical_center_grid[0],
                utm_cell_vertical_center_grid.shape[0:2],
            ),
            np.reshape(
                gps_cell_vertical_center_grid[1],
                utm_cell_vertical_center_grid.shape[0:2],
            ),
        ),
        axis=2,
    ...
    utm_cell_horizontal_center_grid = np.stack(
        (fes(xcornerpoints, ynew), fns(xcornerpoints, ynew) + offset[0]), axis=stackaxis
    ).reshape(len(ynew), len(xcornerpoints), 2)
    gps_cell_horizontal_center_grid = utm.to_latlon(
        utm_cell_horizontal_center_grid[:, :, 0].flatten(),
        utm_cell_horizontal_center_grid[:, :, 1].flatten(),
        utm_center[2],
        utm_center[3],
    )
    gps_cell_horizontal_center_grid = np.stack(
        (
            np.reshape(
                gps_cell_horizontal_center_grid[0],
                utm_cell_horizontal_center_grid.shape[0:2],
            ),
            np.reshape(
                gps_cell_horizontal_center_grid[1],
                utm_cell_horizontal_center_grid.shape[0:2],
            ),
        ),
        axis=2,
    ...
    for i in range(len(ynew)):
        for j in range(len(xnew)):
            utm_grid_waypoints[i, j, 0, 0:2] = utm_cell_horizontal_center_grid[
                i, j, 0:2
            ]
            gps_grid_waypoints[i, j, 0, 0:2] = utm.to_latlon(
                utm_grid_waypoints[i, j, 0, 0],
                utm_grid_waypoints[i, j, 0, 1],
                utm_center[2],
                utm_center[3],
            )
            utm_grid_waypoints[i, j, 1, 0:2] = utm_cell_vertical_center_grid[i, j, 0:2]
            gps_grid_waypoints[i, j, 1, 0:2] = utm.to_latlon(
                utm_grid_waypoints[i, j, 1, 0],
                utm_grid_waypoints[i, j, 1, 1],
                utm_center[2],
                utm_center[3],
            )
            utm_grid_waypoints[i, j, 2, 0:2] = utm_cell_horizontal_center_grid[
                i, j + 1, 0:2
            ]
            gps_grid_waypoints[i, j, 2, 0:2] = utm.to_latlon(
                utm_grid_waypoints[i, j, 2, 0],
                utm_grid_waypoints[i, j, 2, 1],
                utm_center[2],
                utm_center[3],
            )
            utm_grid_waypoints[i, j, 3, 0:2] = utm_cell_vertical_center_grid[
                i + 1, j, 0:2
            ]
            gps_grid_waypoints[i, j, 3, 0:2] = utm.to_latlon(
                utm_grid_waypoints[i, j, 3, 0],
                utm_grid_waypoints[i, j, 3, 1],
                utm_center[2],
                utm_center[3],
            )
...
if __name__ == "__main__":
    ...
    dem_path = os.path.join(
        Path(__file__).parent.absolute(), "..", "data", "20210525_conifer2", "DEM"
    )
    vis = Visualizer(
        dem_path
    )  # initialize an instance of the visualizer (this is optional)

    prob_map = None

    # Optionally define a probability map (as numpy array)
    prob_map = np.ones((np.asarray(area_size) / grid_size).astype(int)) * 0.1
    prob_map[0, 0] = 0.9  # a high probability area is visited first
