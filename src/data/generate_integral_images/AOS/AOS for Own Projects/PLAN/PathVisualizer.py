...
class Visualizer:
    ...
    def __init__(self, lfr_path):
        ...
        if "cornersUTM" in self._info:
            ul_rel_utm = self.relative_utm(
                [(self._info["cornersUTM"][0][0], self._info["cornersUTM"][0][1])]
            )
            lr_rel_utm = self.relative_utm(
                [(self._info["cornersUTM"][1][0], self._info["cornersUTM"][1][1])]
            )
    ...
    def relative_utm(self, utm_pts):
        ...
        for utm_pt in utm_pts:
            ...
            rel_pts.append(
                (
                    utm_pt[0] - self._info["centerUTM"][0],
                    utm_pt[1] - self._info["centerUTM"][1],
                )
            )
        return rel_pts
    ...
    def draw_points(self, pts, format="latlon", color="red", marker="o"):
        ...
        plt.scatter(
            [pt[0] for pt in rel_pts], [pt[1] for pt in rel_pts], c=color, marker=marker
        )

    def draw_lines(
        self, pts, format="latlon", color="black", marker=".", linestyle="-"
    ):
        rel_pts = self.rel_pts(pts, format)
        plt.plot(
            [pt[0] for pt in rel_pts],
            [pt[1] for pt in rel_pts],
            color=color,
            marker=marker,
            linestyle=linestyle,
        )
