...
class CameraControl:
    ...
    def __init__(
        ...
        out_folder="CameraFrame_results",
        cameraindex=0,
        desiredresx=640,
        desiredresy=480,
    ):
    ...
    def AcquireFrames(self, FrameQueue, CameraProcessEvent, GetFramesEvent):
        ...
        self._log = setup_logger(
            "CameraInterpolation_Logger",
            os.path.join(self._out_folder, "CameraFrameCapturedLog.log"),
        )
        while not CameraProcessEvent.is_set():
            if self._FlirAttached and not self._addsynthethicimage:
                ...
                if self._cropImageFlag:
                    Framecropped = Frame[
                        self._cropImage[0] : self._cropImage[0] + self._cropImage[2],
                        self._cropImage[1] : self._cropImage[1] + self._cropImage[3],
                    ]
                ...
                Frame = np.random.randint(
                    0, 255, size=(self._image_res_y, self._image_res_x, 3)
                ).astype(np.uint8)
                CurrentFrametime = datetime.now(timezone.utc).timestamp()
                self._FrameList.append(Frame)
                self._FrameTimeList.append(CurrentFrametime)
...
if __name__ == "__main__":
    ...
    while abs(CurrentTime - StartCameraProcessTime) < 50:
        MainLog.debug(
            " Time Difference %s", str(abs(CurrentTime - StartCameraProcessTime))
        )
        time.sleep(0.2)
        GetFramesEvent.set()
        time.sleep(0.02)
        if not FrameQueue.empty():
            ...
            for i in range(len(FramesInfo["FrameTimes"]) - 1):
                ...
                TimeDifferencebetweenFrames.append(
                    abs(FrameTimeList[i] - FrameTimeList[i + 1])
                )
            if len(TimeDifferencebetweenFrames) > 1:
                MainLog.debug("%s", str(len(FramesInfo["FrameTimes"])))
                MainLog.debug(
                    "Minimum Time Difference between frames %s",
                    str(min(TimeDifferencebetweenFrames)),
                )
                MainLog.debug(
                    "Maximum Time Difference between frames %s",
                    str(max(TimeDifferencebetweenFrames)),
                )
                MainLog.debug(
                    "Mean Time Difference between frames %s",
                    str(np.array(TimeDifferencebetweenFrames).mean()),
                )
                MainLog.debug(
                    "Meadian Time Difference between frames %s",
                    str(statistics.median(np.array(TimeDifferencebetweenFrames))),
                )
        CurrentTime = datetime.now(timezone.utc).timestamp()
    print(
        "50 Seconds Done and CameraAcquireFramesProcess is alive",
        CameraAcquireFramesProcess.is_alive(),
    )
