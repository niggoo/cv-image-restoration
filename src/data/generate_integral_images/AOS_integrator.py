...
def create_integral_image(image_paths: List[str], focal_plane: float) -> np.ndarray:
    """
    Create an integral image from a set of images and focal plane.
    ...
    for img in sorted(image_paths, key=numericalSort):
        n = cv2.imread(img)
        imagelist.append(n)
