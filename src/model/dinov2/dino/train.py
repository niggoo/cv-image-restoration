...
def train_model(
    ...
    experiment = wandb.init(
        project="Custom_dinov2", resume="allow", anonymous="must", entity="pauldd"
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f"Epoch {epoch}/{epochs}", unit="img") as pbar:
            for batch in train_loader:
                ...
                integral_images = integral_images.to(
                    device=device,
                    dtype=torch.float32,
                        ...
                        experiment.log(
                            {
                                ...
                                "output": {
                                    ...
                                    "pred": wandb.Image(
                                        prediction.argmax(dim=1)[0].float().cpu()
                                    ),
                                },
                                "step": global_step,
                                "epoch": epoch,
...
def get_args():
    ...
    parser.add_argument(
        "--in_channels", "-c", type=int, default=4, help="Number of input channels"
    )
