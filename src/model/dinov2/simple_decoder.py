...
class SimpleDecoder(torch.nn.Module):
    def __init__(
        self, in_channels=384, tokenW=36, tokenH=36, num_labels=1, image_size=(512, 512)
    ):
        ...
        self.conv1 = torch.nn.Conv2d(
            in_channels=in_channels, out_channels=4, kernel_size=1, stride=1
        )
    ...
    def forward(self, embeddings):
        ...
        embeddings = embeddings.reshape(
            batch_size, focal_planes, self.height, self.width, hidden_size
        )
        embeddings = embeddings.permute(
            0, 1, 4, 2, 3
        )  # from height, width, hidden_size to hidden_size, height, width
        ...
        pred = torch.nn.functional.interpolate(
            pred, size=self.image_size, mode="bilinear", align_corners=False
        )
